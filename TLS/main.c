...
int main(int argc, char **argv)
{
    ...
    /* Splash */
    printf("\ntlsecho : Simple TLS Echo Client/Server : %s : %s\n\n", __DATE__,
    __TIME__);
    ...
    if (isServer) {
        ...
        while (server_running) {
            ...
            /* Wait for TLS connection from the client */
            if (SSL_accept(ssl) <= 0) {
                ERR_print_errors_fp(stderr);
                server_running = false;
            } else {

                printf("Client TLS connection accepted\n\n");
    ...
    /* Else client */
        ...
        if (SSL_connect(ssl) == 1) {
            ...
            printf("TLS connection to server successful\n\n");
            ...
            printf("TLS connection to server failed\n\n");
